...
class Technology:
    ...
    def distribution_network(self, connections, energy_per_cell, grid_cell_area, base_to_peak_load_ratio,
        ...
        if self.standalone:
            ...
            no_of_service_transf = np.ceil(
                np.maximum(s_max / self.service_transf_type, np.maximum(np.where(self.max_nodes_per_serv_trans !=0,
                                                                                 total_nodes / self.max_nodes_per_serv_trans,
                                                                                 0),
    ...
    def td_network_cost(self, people, new_connections, prev_code, total_energy_per_cell, energy_per_cell,
        ...
        cluster_mv_lines_length_total, cluster_lv_lines_length_total, no_of_service_transf_total, \
            ...
            self.distribution_network(round(new_connections), total_energy_per_cell, grid_cell_area,
...
class SettlementProcessor:
    ...
    def elec_extension_numba(self, grid_calc, sa_diesel_calc, grid_reliability_option, max_dist, year, start_year,
        ...
        if (len(unelectrified) > 0) & (grid_connect_limit > 0):
            iterate = True
            i = 0
        else:
            iterate = False
    ...
    def pv_hybrids_lcoe_lookuptable(self, year, time_step, end_year, mg_pv_hybrid_specs, pv_path=r'../test_data'):
        ...
        ghi_curve, temp = read_environmental_data(pv_path)#, temp_col = 'temperature')# Djibouti ,skiprows=2, temp_col='temperature')
    ...
    def results_columns(self, techs, tech_codes, year, time_step, prio, auto_intensification, mg_interconnection=False,
                        shares:dict[str,float] = {}, assign_shares:bool = True, priorities:list[str] = [], 
                        priority:bool = True):
        ...
        if assign_shares:
            self.df[SET_MIN_OVERALL + "{}".format(year)] = 'None'
            self.df[SET_MIN_OVERALL_LCOE + "{}".format(year)] = 9999

            result_tech = SET_MIN_OVERALL + "{}".format(year)
            result_LCOE = SET_MIN_OVERALL_LCOE + "{}".format(year)

            total_pop = self.df['Pop' + "{}".format(year)].sum()
            tech_target_pop = {tech: int(share * total_pop) for tech, share in shares.items()}
            unassigned_ids = self.df.index.copy()
            tech_target_pop_unassigned = tech_target_pop.copy()

            print('Initial targets:', tech_target_pop_unassigned)

            assigned_existing_grid = self.df[self.df['Technology2024'] == "Existing grid"]
            assigned_existing_grid[result_tech] = "Grid" + "{}".format(year)
            assigned_existing_grid_ids = assigned_existing_grid.index.to_list()
            self.df.loc[assigned_existing_grid_ids, result_tech] = "Grid" + "{}".format(year)
            self.df.loc[assigned_existing_grid_ids, result_LCOE] = assigned_existing_grid["Grid" + "{}".format(year)]
            assigned_existing_grid_pop = assigned_existing_grid["Pop" + "{}".format(year)].sum()

            tech_target_pop_unassigned['Grid'] -= assigned_existing_grid_pop
            if tech_target_pop_unassigned['Grid'] < 0:
                print('Grid rate already reached in the baseline.')

            unassigned_ids = unassigned_ids[~unassigned_ids.isin(assigned_existing_grid_ids)]
            tech_target_pop_unassigned = {k: v for k, v in tech_target_pop_unassigned.items() if v > 0}
            print('\nGrid share initially gridded settlements ', assigned_existing_grid.groupby(result_tech)['Pop'+"{}".format(year)].sum() / self.df['Pop' + "{}".format(year)].sum())
            print('\n', tech_target_pop_unassigned)
            ...
            if priority:
                for tech in priorities:
                    ...
                    condition = self.df[result_tech] == 'None'
                    available_cells = self.df[condition].copy()
                    all_techs = [f'{tech}{year}']
                    if len(available_cells) == 0:
                        tech_target_pop_unassigned[tech] = -999
                        print('No available cells to assign, share cannot be reached.\n')
                        continue
                    available_cells[result_tech] = available_cells[all_techs].T.idxmin()
                    available_cells[result_LCOE] = available_cells[all_techs].T.min()

                    condition = ((available_cells[result_tech] == f"{tech}{year}") & (available_cells[result_LCOE] < 99))
                    candidates = available_cells.loc[condition]
                    print('Available cells: ', len(available_cells),'\nCandidates: ',len(candidates))

                    if len(candidates) == 0:
                        if (available_cells[f'MG_Hydro{year}'] == 99).all():
                            tech_target_pop_unassigned[tech] = -999
                            print('No candidates to assign, since remaining cells are unavailable for this technology.\n')
                            continue
                        else:
                            print('No candidates to assign in this iteration.\n')

                    candidates = candidates.sort_values(result_LCOE, ascending=True)
                    candidates['cummulative_pop'] = candidates['Pop' + "{}".format(year)].cumsum()
                    ...
                    if candidates['Pop' + "{}".format(year)].sum() > target_pop_assign:
                    ...
                    assigned_pop = assigned['Pop' + "{}".format(year)].sum() # Gets the population of the assigned candidates.

                    self.df.loc[assigned_ids, result_tech] = tech + "{}".format(year) # Assigns the technology to the assigned candidates.
                    self.df.loc[assigned_ids, result_LCOE] = assigned[result_LCOE] # Assigns the maximum net benefit to the assigned candidates.
                ...
                tech_target_pop_unassigned = {k: v for k, v in tech_target_pop_unassigned.items() if v > 0}
                print(tech_target_pop_unassigned)
            
            while len(tech_target_pop_unassigned) > 0:
                all_techs = []
                for t in tech_target_pop_unassigned.keys():
                    if t == "MG":
                        mg_cols = [c for c in self.df.columns if c.startswith("MG_") and c.endswith(str(year))]
                        all_techs.extend(mg_cols)
                    else:
                        all_techs.append(f"{t}{year}")
                print('All techs considered this iteration:', all_techs)
                for tech, pop_unassigned in tech_target_pop_unassigned.items():
                    print(f'\nAssigning {tech} with target population: ', f"{pop_unassigned:.2f}")
                    target_pop_assign = pop_unassigned
                    condition = self.df[result_tech] == 'None'
                    available_cells = self.df[condition].copy()
                    ...
                    if tech == "MG":
                        print('Mini-grid columns considered:', mg_cols)
                        available_cells[result_tech] = available_cells[all_techs].T.idxmin()
                        available_cells[result_LCOE] = available_cells[all_techs].T.min()
                        condition = available_cells[result_tech].isin(mg_cols) & (available_cells[result_LCOE] < 99)
                    else:
                        available_cells[result_tech] = available_cells[all_techs].T.idxmin()
                        available_cells[result_LCOE] = available_cells[all_techs].T.min()
                        condition = ((available_cells[result_tech] == f"{tech}{year}") & (available_cells[result_LCOE] < 99))
                    ...
                    print('Available cells: ', len(available_cells),'\nCandidates: ',len(candidates))
                    if len(candidates) == 0:
                        # if (available_cells[f'{tech}{year}'] == 99).all(): 
                        if tech != "MG" and (available_cells[f'{tech}{year}'] == 99).all():
                            tech_target_pop_unassigned[tech] = -999
                            print('No candidates to assign, since remaining cells are unavailable for this technology.\n')
                            continue
                        elif tech == "MG" and np.all(available_cells[mg_cols].values == 99):
                            tech_target_pop_unassigned[tech] = -999
                            print('No candidates to assign, since remaining cells are unavailable for this technology.\n')
                            continue
                        else:
                    ...
                    assigned = candidates[candidates['cummulative_pop'] <= target_pop_assign] # Selects the candidates that are below the target population.
                    print('Assigned before checking last cell: ', len(assigned))
                    if candidates['Pop' + "{}".format(year)].sum() > target_pop_assign:
                        len_assigned = [len(assigned)]
                        assigned = pd.concat([assigned, candidates.iloc[len_assigned]])
                        print('Share completed assigned plus one cell.')
                    ...
                    self.df.loc[assigned_ids, result_tech] = assigned[result_tech] # Assigns the technology to the assigned candidates.
                    self.df.loc[assigned_ids, result_LCOE] = assigned[result_LCOE] # Assigns the maximum net benefit to the assigned candidates.
                    unassigned_ids = unassigned_ids[~unassigned_ids.isin(assigned_ids)] # Updates the unassigned ids.
                    tech_target_pop_unassigned[tech] -= assigned_pop # Updates the target population of the technology.
                    print(f'Assigned population to {tech}: ', f"{assigned_pop:.2f}", 'Remaining population: ', f"{tech_target_pop_unassigned[tech]:.2f}\n")
                ...
                tech_target_pop_unassigned = {k: v for k, v in tech_target_pop_unassigned.items() if v > 0}
                print('Remaining population to assign after iteration:\n',tech_target_pop_unassigned)

            print('\nFinal shares after assignation attempt:', self.df.groupby(result_tech)['Pop' + "{}".format(year)].sum() / self.df['Pop' + "{}".format(year)].sum())

            if len(self.df[self.df[result_tech] == 'None']) > 0:
                condition = self.df[result_tech] == 'None'
                available_cells = self.df[condition].copy()
                all_techs = []
                for t in tech_target_pop.keys():
                    if t == "MG":
                        mg_cols = [c for c in self.df.columns if c.startswith("MG_") and c.endswith(str(year))]
                        all_techs.extend(mg_cols)
                    else:
                ...
                assigned_ids = available_cells.index.to_list()
                self.df.loc[assigned_ids, result_tech] = available_cells[result_tech] # Assigns the technology to the assigned candidates.
                self.df.loc[assigned_ids, result_LCOE] = available_cells[result_LCOE]
            
                print('\nFinal shares after clearing Non-assigned cells:', self.df.groupby(result_tech)['Pop' + "{}".format(year)].sum() / self.df['Pop' + "{}".format(year)].sum())
            ...
            print('Initially grid-connected settlements future share:', 
                  self.df[self.df[SET_ELEC_FINAL_CODE + "{}".format(year - time_step)] < 3]['Pop' + "{}".format(year)].sum() / self.df['Pop' + "{}".format(year)].sum())
            print('Initial grid share', )
        ...
        all_techs = [f"{t}{year}" for t in techs]
    ...
    def calculate_investments_and_capacity(self, sa_pv_investment, sa_pv_capacity, mg_pv_hybrid_investment,
        ...
        self.df[f'Grid_investment{year}'] = grid_investment
        self.df[f'SA_PV_investment{year}'] = sa_pv_investment
        self.df[f'MG_PV_Hybrid_investment{year}'] = mg_pv_hybrid_investment
        self.df[f'MG_Wind_investment{year}'] = mg_wind_investment
        self.df[f'MG_Hydro_investment{year}'] = mg_hydro_investment
        ...
        self.df['Grid_capacity' + "{}".format(year)] = grid_capacity
        self.df['SA_PV_capacity' + "{}".format(year)] = sa_pv_capacity
        self.df['MG_PV_Hybrid_capacity' + "{}".format(year)] = mg_pv_hybrid_capacity
        self.df['MG_Wind_capacity' + "{}".format(year)] = mg_wind_capacity
        self.df['MG_Hydro_capacity' + "{}".format(year)] = mg_hydro_capacity
        
    def pre_selection(self, eleclimit, year, time_step, prioritization, auto_densification=0, prio_choice=1):
        ...
        if eleclimit == 1:
            ...
            self.df['Intensification'] = np.where((self.df[SET_MV_DIST_PLANNED] < auto_densification) & (self.df['MaxIntensificationDist']), 0, 1)
